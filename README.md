...
```bash
# Run the two configs (naive baseline vs tuned production prompt) end-to-end,
# with the judge, then compare + gate + emit the demo-site data in one shot:
python scripts/run_ab_demo.py

# Or run/compare/gate individually:
python scripts/run_experiment.py v2                 # tuned config, judged
...
### A/B result: naive prompt → tuned prompt

Same pipeline and same 100-question golden set; only the system prompt
changed (`app/prompts.py`: `v1_naive` → `v2_tuned`). The naive prompt has no
escalation policy and no prompt-injection guardrails, so it over-answers the
account-specific, security, legal, and out-of-scope tickets it should hand
off. The harness catches it:
